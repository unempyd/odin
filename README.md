...
|---|---|---|---|---|
| Safe default: a fresh profile launches every worker with no bypass flag in its argv (`ps` captured live) | no bypass | no bypass; parks on its own approval prompt and Orca reports the wait, not success | no bypass | `real-sessions.2026-09-14T14-29-05-741Z.json` |
...
| Concurrent: two workers started in parallel both settle | 8.6 s | 4.5 s | — | same file |
| Crash mid-send: host SIGKILLed after the send, restarted on the same profile; replaying the same `--retry-request` returns the identical dispatch (one row, no second worker); the worker is still live, never falsely exited; Codex and Grok then report honestly after the restart | pass | pass | pass | `real-sessions.2026-09-14T14-49-45-377Z.json` |
