...
<Callout title="Odin has no store builds">
  This page describes Orca's mobile companion, which is part of the same
  product surface Odin runs unchanged. Odin itself ships no App Store,
  TestFlight, or Android APK build, and no hosted pairing/relay service —
  it is source-only. See [Odin's GitHub
  releases](https://github.com/unempyd/odin/releases) for what is
  published, and [What is Odin?](/docs/odin) for why. If Orca's own mobile
  builds are what you want, they remain available from Stably AI's App
  Store, TestFlight, and APK links on
...
- **Update Orca Mobile / Update Orca on your computer** — the phone and desktop speak a versioned mobile protocol. If either side is too old, mobile blocks that host and points you at an update source appropriate to that build (Stably AI's App Store listing or [GitHub Releases](https://github.com/stablyai/orca/releases) for Orca; [GitHub Releases](https://github.com/unempyd/odin/releases) for Odin, source-only), then back to hosts. After updating, refresh the connection; if the message stays, remove the host and pair again.
