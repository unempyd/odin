...
<Callout title="Odin: fresh installs are opted out by default">
  Orca upstream wrote `optedIn: true` for every new install's telemetry
  migration without ever showing the consent banner — only pre-existing
  installs saw the first-launch surface. On Odin a fresh install is opted
  out by default; a pre-existing profile that Orca had already auto-enrolled
  keeps that choice until you change it below. See [Odin's residual table,
  contract F](/docs/odin/residuals) (commit `aacecbdaf4`).
</Callout>
...
- **Always off when:** `DO_NOT_TRACK=1` or `ORCA_TELEMETRY_DISABLED=1` is set, or (Odin, fresh installs only) the install has never had this setting explicitly turned on.
