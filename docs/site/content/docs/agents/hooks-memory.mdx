...
<Callout title="Odin: opt-in, not opt-out">
  On Odin this setting defaults to **off**, and an absent, null, or unreadable
  setting is also treated as off — Orca upstream defaulted it to on and
  treated anything other than an explicit `false` as on, which installed
  hooks into 14 other tools' user-global configs on a fresh profile. See
  [Odin's residual table, contract E](/docs/odin/residuals) (commit
  `29624f65b9`).
</Callout>
