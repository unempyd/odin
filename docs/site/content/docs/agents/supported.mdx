...
<Callout title="Odin: shipped defaults carry no bypass flag">
  Orca upstream's shipped default for 26 agents *was* the bypass flag —
  `DEFAULT_TUI_AGENT_ARGS = YOLO_TUI_AGENT_ARGS` — and force-migrated it into
  profiles that never chose it. On Odin, shipped defaults carry no bypass
  flag; the migration hydrates the same keys with empty values instead. A
  one-shot review migration also clears any inherited grant that still
  equals Orca's former automatic value verbatim (a user who explicitly chose
  that same value can turn it back on once). The rest of this page describes
  the **Yolo** permission mode itself, which is unchanged and still opt-in
  per agent or globally — see [Odin's residual table, contract
  H](/docs/odin/residuals) (commits `854b5956ba`, `74b234c61e`).
</Callout>

<Callout title="Permission safety">
  Choosing **Yolo** passes each agent's permission-bypass flag for new launches. A worktree is an
