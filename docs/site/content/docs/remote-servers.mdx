...
<Callout title="Odin: the listener needs consent to widen, every time">
  Orca upstream widened the runtime WebSocket listener from loopback to
  `0.0.0.0` on every start once any device had ever paired, and never
  narrowed it back. On Odin the listener stays on loopback unless a
  persisted `networkExposureConsent` record is `true`, re-read on every
  bind; generating an access link or advertising as a server refuses to
  widen it without that consent, and turning the setting off rebinds the
  live listener straight back to loopback. `orca serve` and `orcad --bind`
  remain explicit opt-ins either way. See [Odin's residual table, contract
  D](/docs/odin/residuals) (commit `6d10576a71`).
</Callout>
