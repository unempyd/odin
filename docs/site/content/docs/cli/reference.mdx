...
<Callout title="Odin: terminal wait --for tui-idle reports its evidence">
  Orca upstream's tier-3 "sustained title idle" check returned the same
  `true` as positive evidence of readiness — a silent worker could be
  declared ready or an automation run declared completed on nothing but
  absence. On Odin, `terminal wait --for tui-idle` resolves a three-valued
  verdict (`observed-idle` / `silence` / `not-idle`); the JSON result carries
  an `evidence` field describing what was actually observed, and `satisfied`
  is never `true` on silence alone. Two tier-1 exceptions still count as
  positive evidence: an explicit idle marker or ready prompt the agent
  itself paints, and — because it is their only rest signal — the
  quiescence window for agents named `grok`, `copilot`, `aider`, `mimo`,
  `agy`, and `opencode`. See [Odin's residual table, contract
  C](/docs/odin/residuals) (commit `07522244e6`).
</Callout>
