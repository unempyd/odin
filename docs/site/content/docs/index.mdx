...
<Callout title="You're reading Odin's docs">
  This page and everything under **Docs** describe Orca's product surface,
  which Odin runs unchanged. Odin is a derivative that closes a bounded set
  of residual safety and orchestration issues — see [What is
  Odin?](/docs/odin) for what's different and why, and look for a one-line
  note on any page whose described default Odin changed.
</Callout>
