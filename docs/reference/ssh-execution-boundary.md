...
This is the vocabulary for a PTY's own process liveness as observed through the execution-host boundary this document describes — it does not reach every liveness-shaped enum in the codebase. Two adjacent, pre-existing vocabularies answer different local-only questions and are untouched: `daemon-audit-classifier.ts:25-69`'s `present | gone | unknown` audits whether the *local terminal daemon process itself* is still running (a health check on Orca's own daemon, not a worker's PTY) and defaults to `unknown` when its probe cannot conclusively decide; `orca-runtime-get-worktree-ps.ts:230-244`'s `live | dead | unknown` recovers a *structured-agent-session TUI owner's local PID* after a restart — no relay or SSH transport involved — and also defaults to `unknown` on an inconclusive probe, except when no owner PID was ever recorded (`!identity`), which answers `dead` outright: there is nothing to probe, not an ambiguous result to preserve.
...
**`terminal_gone` is overloaded across two evidentiary strengths.** The same string means different things depending on which layer throws it. `relay/pty-handler.ts:2664`'s throw is a real relay-side claim: this pty id is not in the relay's own session/tombstone bookkeeping, evidence a client on the other end of the wire could not fabricate. `pty-process-inspection.ts:34-36` (`provider.hasPty(ptyId) === false`), `daemon-pty-router.ts:326-333` (`adapterForInspection` finds no adapter that claims the id), and `degraded-daemon-pty-provider.ts:187-190` (`hasPty` false) are a different claim: this process's own in-memory registry has no record of the id, true whether the pty never existed, already exited, or was never indexed. `worker-observation.ts`'s O1 fix (the shared `isOwnerProvenTerminalAbsence` gate in `shared/terminal-process-inspection.ts`, which `federation-attachment-observation.ts`'s `inspectRemoteAttachment` applies too since O4) deliberately treats every one of these throws as equally owner-proven `missing`, because from main's vantage the daemon it just asked about a local pty *is* that pty's owner — a miss on your own bookkeeping is the owner's answer, not a lesser one. Two client-side consumers downgrade instead: `classifyTerminalProcessInspectionFailure` (`shared/terminal-process-inspection.ts:106-165`, fixed under O3) used to bucket `terminal_gone` together with `terminal_handle_stale`, `terminal_exited`, `no_connected_pty`, and a `PTY "<id>" not found` message under one client-facing reason — the same overloading this section warns about, just one layer further from the owner. It now gives each of those four client-side shapes its own reason (`terminal_handle_stale`, `terminal_exited`, `no_connected_pty`, `terminal_not_found`) and reserves `terminal_gone` for the one case that is actually the host's own claim (`code === 'terminal_gone'` or a message naming it); `runtime-terminal-inspection.ts:96,117,314` / `terminal-identity.ts:104,142,145` still read every one of the five reasons as `unverifiable` (or `old_host` across a version boundary), never as proof — the renderer and CLI are one hop further from the terminal's actual owner than main is, so none of these five reasons earns more trust there than any other. The distinct spellings exist so a future consumer that only wants the host's own claim (as `isOwnerProvenTerminalAbsence` already does for both observers, matching the raw code/message directly rather than going through this classifier) can ask for `terminal_gone` alone instead of inheriting the whole bucket.

**`terminal show`'s `connected`/`writable` must track the owning target's transport, not just the PTY record.** `buildPtyTerminalSummary`/`buildTerminalSummary` (`src/main/runtime/orca-runtime-build-pty-terminal-summary.ts`, `orca-runtime-write-orchestration-pointer-pty.ts`) otherwise read `pty.connected` straight off the record, which a PTY data/exit event or an inventory sweep updates — never the relay-lost/reconnect callbacks in `src/main/ipc/ssh-relay-session-callbacks.ts`. Proven against a real host (`odin/proofs/ssh-boundary.2026-09-15T01-39-52-596Z.json` phase4): after an `iptables`-dropped transport, `[ssh] Relay channel ... lost; reconnect attempt 1/6` and `Keepalive timeout` fired while `terminal show` kept answering `connected: true, writable: true` with no `exitCause` for all 17 samples over 90s — silence read as life, the same error this document exists to prevent, just in the other direction. The fix reuses the existing SSH connection-state registry (`getRegisteredSshState`, `src/main/ssh/ssh-target-registry.ts`) rather than adding a second liveness tracker: while the owning target's status is not `connected`, `connected`/`writable` are forced `false` and the record carries `liveness: { status: 'unverifiable', reason: 'reconnecting' | 'transport_lost' }` — never `exitCause`, never `exited`. A genuine owner-proven exit (`pty.lastExitCause` already set) is never overridden by transport status. See `show-contact-loss` in `odin/proof/manifest.json`.
