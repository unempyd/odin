...
export type RuntimeTerminalSummary = {
  ...
  /** Present only while the owning SSH target's relay/connection is not `connected`: forces
   *  `connected`/`writable` false above and is never set alongside `exitCause` — loss of contact
   *  is `unverifiable`, never `exited` (docs/reference/ssh-execution-boundary.md). Absent for
   *  local terminals and once the target reads `connected` again. */
  liveness?: { status: 'unverifiable'; reason: 'transport_lost' | 'reconnecting' }
}
...
// Why split out: these three types are re-exported here unchanged so every existing importer of
// `./runtime-terminal-contracts` (direct or via `shared/runtime-types`) keeps working — moved only
// to stay under this file's line budget after `RuntimeTerminalSummary.liveness` was added.
export type {
  RuntimeTerminalWait,
